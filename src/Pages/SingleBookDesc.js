...
function SingleBookDesc(props) {
  let singleData = 'boş';
  if (typeof (props.route.params.singleBookData) !== 'undefined' &&
    ...
    <SafeAreaView style={styles.mainContainer}>
      ...
      <View style={{ flex: 0.5, flexDirection: 'row', height: '70%', width: '95%', padding: 10, justifyContent: 'flex-start', marginLeft: 10, marginTop: 10 }}>
        ...
        <View style={{ justifyContent: 'flex-end', backgroundColor: 'white', flex: 1.4, padding: 10, borderTopRightRadius: 10, borderBottomRightRadius: 10 }}>
          <Text style={{ marginBottom: 7, color: "#575758", fontWeight: 'bold' }}>{bookController.checkTitle(singleData)}</Text>
          <Text style={{ marginBottom: 10, color: "#A0A0A1", fontSize: 10 }}>{bookController.checkAuthor(singleData)}</Text>
          ...
          <Rating
            ...
            direction="row"
          />
          <Text style={{ marginBottom: 5, marginTop: 10, fontSize: 10 }}>{bookController.checkDescription(singleData)}</Text>
